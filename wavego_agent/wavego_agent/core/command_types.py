...
class LightColor(Enum):
    """LED light colors (matches robot.py)."""
    OFF = "off"
    BLUE = "blue"
    RED = "red"
    GREEN = "green"
    ...
    CYAN = "cyan"
    MAGENTA = "magenta"
    CYBER = "cyber"  # Original name from robot.py (white/rainbow effect)
...
def create_light_command(color: LightColor, light_id: int = 0) -> MotorCommand:
    """
    Create an LED light motor command.
    
    Args:
        color: Light color
        light_id: Which light (0=front, 1=back)
    """
    var_name = "light" if light_id == 0 else "lightB"
    return MotorCommand(
        command_type=var_name,
