...
class Agent:
    ...
    def _handle_light(self, action: str, color: str) -> List[MotorCommand]:
        ...
        WaveGo has two lights (0=front, 1=back).
        By default, sets both lights to the same color.
        """
        color_map = {
            "off": LightColor.OFF,
            ...
            "white": LightColor.CYBER,  # Alias
        }
        
        light_color = color_map.get(color.lower(), LightColor.BLUE)
        
        # Set both lights to the same color
        return [
            create_light_command(light_color, light_id=0),
            create_light_command(light_color, light_id=1)
        ]
