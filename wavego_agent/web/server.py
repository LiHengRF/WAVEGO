    ...
    def update_connection_state(self):
        ...
        with self.lock:
            ...
            if self.motor_controller:
                try:
                    self.state.serial_connected = self.motor_controller.is_connected()
                except:
                    self.state.serial_connected = False
            
            # Vision processor (camera) status - use _is_running
            if self.vision_processor:
                self.state.camera_active = getattr(
                    self.vision_processor, '_is_running', False
                )
            
            # LLM connection
            if self.agent and hasattr(self.agent, 'llm'):
                self.state.llm_connected = self.agent.llm is not None
            
            # STT status
            if self.stt:
                self.state.stt_active = getattr(self.stt, '_is_listening', False)
...
def set_light(color: str):
    """Set LED light color"""
    valid_colors = ['red', 'green', 'blue', 'yellow', 'cyan', 'magenta', 'white', 'cyber', 'off']
    
    if color not in valid_colors:
        return jsonify({'success': False, 'error': f'Invalid color: {color}'})
    
    # Get light_id from request (default 0 for backward compatibility)
    data = request.get_json() or {}
    light_id = data.get('light_id', 0)
    
    dashboard.add_log('info', 'web', f'Light {light_id} color: {color}')
    dashboard.update_robot_state(light_color=color)
    
    if dashboard.motor_controller:
        try:
            dashboard.motor_controller.light_ctrl(color, light_id)
            return jsonify({'success': True})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
    else:
...
@app.route('/api/light/<int:light_id>/<color>', methods=['POST'])
def set_light_by_id(light_id: int, color: str):
    """Set specific LED light color by light ID"""
    valid_colors = ['red', 'green', 'blue', 'yellow', 'cyan', 'magenta', 'white', 'cyber', 'off']
    
    if color not in valid_colors:
        return jsonify({'success': False, 'error': f'Invalid color: {color}'})
    
    if light_id not in [0, 1]:
        return jsonify({'success': False, 'error': f'Invalid light_id: {light_id}'})
...
@app.route('/api/buzzer/<state>', methods=['POST'])
def set_buzzer(state: str):
    """Control buzzer"""
    if state not in ['on', 'off']:
        return jsonify({'success': False, 'error': f'Invalid state: {state}'})
    
    on = (state == 'on')
    dashboard.add_log('info', 'web', f'Buzzer: {state}')
    
    if dashboard.motor_controller:
        try:
            dashboard.motor_controller.buzzer_ctrl(on, 0)
            return jsonify({'success': True})
        except Exception as e:
            return jsonify({'success': False, 'error': str(e)})
    else:
